...
export const HighImpactHero: React.FC<Page['hero']> = ({  media, buttonTitle, title="The Simple Way to Collect Contributions", subTitle="Build trust with real time records and digital receipts for every contribution", avatarsSection }) => {
    ...
    <div className='w-full px-2 max-w-[2080px] mx-auto grid grid-cols-1 gap-1 lg:grid-cols-2 mt-10 md:mt-15'>
      <div className='bg-dark rounded-xl p-10 lg:p-26 text-white' >
         <div className='w-full xl:max-w-[80%]'>
          <h1 className='text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-7xl font-bold font-chillax '>{title}</h1>
          <p className='my-4 xl:max-w-[60%]'>{subTitle}</p> 
          <DownloadButton variant='invert' title={buttonTitle || 'Download App'} />
         </div>

          <div className='flex  flex-col md:flex-row w-full mt-30 lg:mt-50 md:items-center gap-4 justify-between'>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex -space-x-3">
                    {avatarsSection.avatars.slice(0, 5).map((avatar, index) => (
                      <Avatar key={index} className="w-10 h-10 border-2 border-secondary-background">
      ...
      <div className='bg-secondary-background rounded-xl h-[400px] lg:min-h-full p-20 text-white overflow-hidden relative'>
        ...
        {media && typeof media === 'object' && (
          <Media fill imgClassName="z-10 object-contain object-bottom lg:pt-20 pl-20" priority resource={media} />
        )}
      </div>
    </div>
  )
