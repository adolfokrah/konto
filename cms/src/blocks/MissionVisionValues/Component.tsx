...
import { Lightbulb, Target, Gem, Heart, Crosshair, Star } from 'lucide-react'
...
> = ({ title, items, className, anchor }) => {
  return (
    <section className={cn(' px-5 py-16 m-auto ', className)} id={anchor || undefined}>
              ...
              {items.map((item: MissionVisionValuesItem, index: number) => (
                ...
                >
                  ...
                  <div className="max-w-[341px]">
                    ...
                    <p className="lg:leading-[27px]text-center font-medium">{item.description}</p>
                  </div>
                </div>
              ))}
