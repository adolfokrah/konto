...
const CURRENCY = 'USD'
const PRESET_AMOUNTS = [100, 50, 20, 10, 5, 1]
const DEFAULT_AMOUNT = 10

interface ContributionInputInternationalProps {
...
export default function ContributionInputInternational({
  isFixedAmount = false,
  fixedAmount = 0,
  className = '',
  jarId = '',
  ...
  customFields = [],
}: ContributionInputInternationalProps) {
  const [selectedAmount, setSelectedAmount] = useState<number>(
    isFixedAmount ? fixedAmount : DEFAULT_AMOUNT,
  )
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
        ...
        if (res.ok) {
          ...
          if (data.success) {
            ...
            setCharges({
              ...
              minimumContributionAmount: data.minimumContributionAmount ?? 1,
            })
            return
          }
        }
      ...
      // Fallback: 3.9% + $0.30 (typical international card rate)
      const fee = Math.round((selectedAmount * 0.039 + 0.3) * 100) / 100
      setCharges({
        platformCharge: fee,
  ...
  const totalAmountToPay = charges?.amountPaidByContributor ?? contributionAmount
    ...
    <div className={`bg-white ${className}`}>
      ...
      <div className="font-supreme space-y-2">
        ...
        <div className="flex justify-between text-gray-700">
          ...
          <span>USD {formatAmount(contributionAmount)}</span>
        </div>

        <div className="flex justify-between text-gray-700">
          <span>Processing fee</span>
          <span>USD {formatAmount(transactionFee)}</span>
        </div>

        <div className="flex justify-between font-bold text-black pt-2 border-t border-gray-200">
          <span>Total due to pay</span>
          <span>USD {formatAmount(totalAmountToPay)}</span>
        </div>
      </div>

      {charges?.minimumContributionAmount != null &&
        selectedAmount > 0 &&
        selectedAmount < charges.minimumContributionAmount && (
          <p className="text-sm text-red-500 font-supreme mt-2 mb-2">
            Minimum contribution is USD {charges.minimumContributionAmount}
          </p>
        )}
      ...
      <button
        ...
        disabled={
          selectedAmount <= 0 ||
          selectedAmount < (charges?.minimumContributionAmount ?? 1) ||
          isLoading ||
