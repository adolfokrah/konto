...
interface NewJarsTrendChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily new jars over the last 30 days'
  if (range === 'monthly') return 'Monthly new jars over the last 12 months'
  return 'New jars by year'
}

export function NewJarsTrendChart({ data, range = 'daily' }: NewJarsTrendChartProps) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>New Jars Created</CardTitle>
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <AreaChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 8 }}>
            <defs>
              <linearGradient id="fillJars" x1="0" y1="0" x2="0" y2="1">
            ...
            />
            <YAxis tickLine={false} axisLine={false} tickMargin={8} width={40} domain={[0, (max: number) => max > 0 ? Math.ceil(max * 1.15) : 1]} allowDecimals={false} />
            <Tooltip
