...
interface TotalTransactionVolumeChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily completed volume (contributions + payouts) over the last 30 days'
  if (range === 'monthly') return 'Monthly completed volume over the last 12 months'
  return 'Total completed volume by year'
}

export function TotalTransactionVolumeChart({ data, range = 'daily' }: TotalTransactionVolumeChartProps) {
  return (
    <Card>
      <CardHeader>
