...
interface PayoutVolumeTrendChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily payout amounts over the last 30 days'
  if (range === 'monthly') return 'Monthly payout amounts over the last 12 months'
  return 'Payout amounts by year'
}

export function PayoutVolumeTrendChart({ data, range = 'daily' }: PayoutVolumeTrendChartProps) {
  return (
    <Card>
      <CardHeader>
