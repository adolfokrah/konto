...
interface PayoutCountTrendChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily completed payout count over the last 30 days'
  if (range === 'monthly') return 'Monthly completed payout count over the last 12 months'
  return 'Completed payout count by year'
}

export function PayoutCountTrendChart({ data, range = 'daily' }: PayoutCountTrendChartProps) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Payout Count</CardTitle>
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <AreaChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 8 }}>
            <defs>
              <linearGradient id="fillPayoutCount" x1="0" y1="0" x2="0" y2="1">
