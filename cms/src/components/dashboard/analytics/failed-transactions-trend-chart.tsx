...
interface FailedTransactionsTrendChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily failed transactions over the last 30 days'
  if (range === 'monthly') return 'Monthly failed transactions over the last 12 months'
  return 'Failed transactions by year'
}

export function FailedTransactionsTrendChart({ data, range = 'daily' }: FailedTransactionsTrendChartProps) {
  return (
    <Card>
      <CardHeader>
