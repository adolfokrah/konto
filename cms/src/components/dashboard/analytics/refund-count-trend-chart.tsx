...
interface RefundCountTrendChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily refund count over the last 30 days'
  if (range === 'monthly') return 'Monthly refund count over the last 12 months'
  return 'Refund count by year'
}

export function RefundCountTrendChart({ data, range = 'daily' }: RefundCountTrendChartProps) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Refund Count</CardTitle>
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <AreaChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 8 }}>
            <defs>
              <linearGradient id="fillRefundCount" x1="0" y1="0" x2="0" y2="1">
