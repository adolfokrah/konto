...
interface TransactionCountTrendChartProps {
...
function getDescription(range: Range) {
  if (range === 'daily') return 'Daily transaction count over the last 30 days'
  if (range === 'monthly') return 'Monthly transaction count over the last 12 months'
  return 'Transaction count by year'
}

export function TransactionCountTrendChart({ data, range = 'daily' }: TransactionCountTrendChartProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <AreaChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 8 }}>
            <defs>
              <linearGradient id="fillTxCount" x1="0" y1="0" x2="0" y2="1">
