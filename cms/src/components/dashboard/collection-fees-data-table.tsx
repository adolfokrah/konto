...
import { collectionFeeColumns, type CollectionFeeRow } from './data-table/columns/collection-fee-columns'
...
const empty: FormState = { country: 'ghana', paymentMethodId: '', fee: '', hogapaySplit: '', minimumContributionAmount: '' }

export function CollectionFeesDataTable({
  fees,
  paymentMethods,
}: {
  fees: CollectionFeeRow[]
  paymentMethods: PaymentMethodOption[]
}) {
  const router = useRouter()
  const [, startTransition] = useTransition()
  const [dialogOpen, setDialogOpen] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<CollectionFeeRow | null>(null)
  const [editing, setEditing] = useState<CollectionFeeRow | null>(null)
  ...
  const openEdit = (fee: CollectionFeeRow) => {
    setEditing(fee)
    setForm({
      country: fee.country,
      ...
      minimumContributionAmount: String(fee.minimumContributionAmount),
    })
    setDialogOpen(true)
  }

  const handleSave = async () => {
    if (!form.country || !form.paymentMethodId || form.fee === '' || form.hogapaySplit === '' || form.minimumContributionAmount === '') {
    ...
    try {
      const body = {
        ...
        minimumContributionAmount: parseFloat(form.minimumContributionAmount),
      }
      const res = editing
        ? await fetch(`/api/collection-fees/${editing.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify(body),
          })
        : await fetch('/api/collection-fees', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify(body),
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/collection-fees/${deleteTarget.id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
  ...
  const bulkActions: BulkAction<CollectionFeeRow>[] = useMemo(
    () => [
      {
        label: 'Delete',
        ...
        onClick: async (rows, clearSelection) => {
          await Promise.all(
            rows.map((r) =>
              fetch(`/api/collection-fees/${r.id}`, {
                method: 'DELETE',
                credentials: 'include',
              }),
            ),
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold">Collection Fees</h1>
      ...
      <Card className="flex flex-col flex-1 min-h-0">
        <CardHeader>
          <CardTitle>Collection Fees</CardTitle>
          <CardDescription>
            {fees.length} rate{fees.length !== 1 ? 's' : ''} configured
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col flex-1 min-h-0 overflow-hidden">
          <DataTable
            tableId="collection-fees"
            columns={collectionFeeColumns}
            data={fees}
            emptyMessage="No collection fees configured yet."
            tableMeta={tableMeta}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{editing ? 'Edit Collection Fee' : 'Add Collection Fee'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Min Contribution</label>
              <Input
                type="number"
                step="0.5"
                placeholder="2"
                value={form.minimumContributionAmount}
                onChange={(e) => setForm((p) => ({ ...p, minimumContributionAmount: e.target.value }))}
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(v) => !v && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete collection fee?</AlertDialogTitle>
            <AlertDialogDescription>
