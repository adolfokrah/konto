export default class TransactionCharges {
  private readonly paystackFee = 0.0195 // 1.95%
  private readonly platformFee = 0.02 // 2%
  private readonly transferFee = 1 // 1 cedi
  ...
  calculateContributorAmount(amount: number): { totalAmount: number; paystackCharge: number } {
    // Add Paystack fee to original amount
    const totalAmount = this.roundToTwo(amount * (1 + this.paystackFee))
  ...
  calculateRecipientAmount(originalAmount: number, totalAmount: number, paystackCharge: number) {
    ...
    const roundingDifference = this.roundToTwo(originalAmount - amountAfterPaystackFees)

    // Calculate our platform charge (2% of original amount, adjusted for rounding)
    const basePlatformCharge = originalAmount * this.platformFee - roundingDifference
    ...
    if (basePlatformCharge > this.transferFeeThreshold) {
      ...
      return {
        ...
        amountAfterCharges: this.roundToTwo(baseAmountAfterCharges - this.transferFee),
      }
    }
  }
