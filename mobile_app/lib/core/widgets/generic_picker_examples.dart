...
void showCategoryPicker(BuildContext context) {
  final categories = ['Food', 'Transport', 'Entertainment', 'Shopping', 'Bills'];
  ...
  GenericPicker.showPickerDialog<String>(
    ...
    itemBuilder: (category, isSelected, onTap) => ListTile(
    ...
    recentItemBuilder: (category, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
      title: Text(category, style: TextStyles.titleMedium),
      trailing: const Icon(Icons.history, size: 18),
      onTap: onTap,
    ),
    searchResultBuilder: (category, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
      title: Text(category, style: TextStyles.titleMedium),
      trailing: isSelected ? const Icon(Icons.check, size: 18) : null,
      onTap: onTap,
...
void showPaymentMethodPicker(BuildContext context) {
  final paymentMethods = [
    PaymentMethod(id: 'card', name: 'Credit Card', description: 'Pay with card', icon: Icons.credit_card),
    PaymentMethod(id: 'bank', name: 'Bank Transfer', description: 'Direct bank transfer', icon: Icons.account_balance),
    PaymentMethod(id: 'mobile', name: 'Mobile Money', description: 'Mobile wallet payment', icon: Icons.phone_android),
  ];
  ...
  GenericPicker.showPickerDialog<PaymentMethod>(
    ...
    itemBuilder: (method, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
    ...
    recentItemBuilder: (method, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
      leading: Icon(method.icon, size: 24),
      title: Text(method.name, style: TextStyles.titleMedium),
    ...
    searchResultBuilder: (method, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
      leading: Icon(method.icon, size: 24),
      title: Text(method.name, style: TextStyles.titleMedium),
      subtitle: Text(method.description, style: TextStyles.titleRegularSm),
...
void showUserPicker(BuildContext context) {
  ...
  GenericPicker.showPickerDialog<User>(
    ...
    itemBuilder: (user, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
    ...
    recentItemBuilder: (user, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
      leading: CircleAvatar(
    ...
    searchResultBuilder: (user, isSelected, onTap) => ListTile(
      contentPadding: EdgeInsets.zero,
      leading: CircleAvatar(
